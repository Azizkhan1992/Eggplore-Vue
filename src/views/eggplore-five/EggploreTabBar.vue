  ...
  <div class="eggplore-tabbar-container">
    ...
    <div class="tabbar-content">
      <div class="tabbar" ref="tabChild">
        <div v-for="(header, idx) in tabHeaders" :key="idx">
          <button  :class="{'default-active': header.active}" @click="setActiveTab(header.value)">{{ header.name }}</button>
          <svg
            width="47"
            height="3"
            viewBox="0 0 47 3"
...
export default {
  ...
  computed: {
    activeTab() {
      return this.tabHeaders.length > 0 ? this.tabHeaders.find(item => item.active).value : null;
    }
  },
  methods: {
